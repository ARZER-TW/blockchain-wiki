...
BLS12-381 是 Ethereum 共識層（[Beacon Chain](/ethereum/consensus/beacon-chain/)）使用的 pairing-friendly 橢圓曲線。「BLS」指 Barreto-Lynn-Scott 構造，「12」是 embedding degree，「381」是基域質數的位元數。它支援雙線性配對（bilinear pairing）運算，使 [BLS Signatures](/ethereum/cryptography/bls-signatures/) 的聚合簽名成為可能，也是 [KZG Commitments](/ethereum/advanced/kzg-commitments/) 和 [zkSNARKs 支援](/ethereum/advanced/zksnarks/) 的基礎。
...
Ethereum 的 [BLS Signatures](/ethereum/cryptography/bls-signatures/) 選擇公鑰在 $G_1$（48 bytes），簽名在 $G_2$（96 bytes），因為 Beacon Chain 需要頻繁聚合公鑰。
...
- [橢圓曲線密碼學](/ethereum/cryptography/elliptic-curve-cryptography/) - ECC 的通用原理
