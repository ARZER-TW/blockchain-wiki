...
SHA-256（Secure Hash Algorithm 256-bit）是 SHA-2 家族的成員，由 NSA 設計、NIST 標準化。在 Ethereum 生態中，SHA-256 主要用於 [Beacon Chain](/ethereum/consensus/beacon-chain/) 共識層和 [Precompiled Contracts](/ethereum/advanced/precompiled-contracts/)（地址 0x02），而執行層以 [Keccak-256](/ethereum/cryptography/keccak-256/) 為主。
...
- 比 [Keccak-256](/ethereum/cryptography/keccak-256/) 的 EVM opcode 貴一倍
- 主要用於與 Bitcoin 等系統的互操作

### Beacon Chain（共識層）

- [Beacon Chain](/ethereum/consensus/beacon-chain/) 的 [SSZ 編碼](/ethereum/data-structures/ssz-encoding/) 使用 SHA-256 作為 Merkleization 的雜湊函數
...
- [雜湊函數概述](/ethereum/cryptography/hash-function-overview/) - 雜湊函數的安全性質總覽
...
- [Merkle Tree](/ethereum/data-structures/merkle-tree/) - SHA-256 是常用的雜湊函數選項
