---
title: "EIP-155 重放保護"
description: "EIP-155, EIP-155 重放保護, Chain ID, Replay Protection, Simple Replay Attack Protection"
...
這就是跨鏈重放攻擊。[Nonce](/ethereum/accounts/nonce/) 只防同鏈重放，跨鏈的 nonce 可能恰好匹配。
...
4. 用 [ECRECOVER](/ethereum/cryptography/ecrecover/) 從 `(v, r, s)` 恢復公鑰
5. 公鑰推導地址，比對 `from`
...
- **[交易簽名](/ethereum/transaction-lifecycle/transaction-signing/)**：簽名前將 chainId 納入 hash
- **[交易廣播與驗證](/ethereum/transaction-lifecycle/broadcast-validation/)**：節點驗證 chainId 匹配
- **多鏈開發**：開發者必須確保 chainId 正確
- **[ECDSA](/ethereum/cryptography/ecdsa/)**：v 值的語義改變
...
- [ECDSA](/ethereum/cryptography/ecdsa/) - EIP-155 修改了簽名的 v 值語義
- [ECRECOVER](/ethereum/cryptography/ecrecover/) - 驗證時用 v 值恢復公鑰
- [Nonce](/ethereum/accounts/nonce/) - 防止同鏈重放，EIP-155 防止跨鏈重放
- [交易簽名](/ethereum/transaction-lifecycle/transaction-signing/) - EIP-155 改變了簽名的 hash 輸入
- [交易廣播與驗證](/ethereum/transaction-lifecycle/broadcast-validation/) - 節點驗證 chainId 是否匹配
- [RLP 編碼](/ethereum/data-structures/rlp-encoding/) - 簽名 hash 的輸入使用 RLP 序列化
- [EIP-1559 費用市場](/ethereum/accounts/eip-1559/) - Type 2 交易直接包含 chainId 欄位
- [Keccak-256](/ethereum/cryptography/keccak-256/) - 簽名 hash 使用 keccak256
