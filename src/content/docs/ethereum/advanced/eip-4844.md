...
description: "EIP-4844, Proto-Danksharding, Blob Transaction, Dencun"
...
EIP-4844 定義了新的交易類型 `0x03`，在 [EIP-1559 費用市場](/ethereum/accounts/eip-1559/) 基礎上新增 blob 相關欄位：
...
一個 blob 是一個固定大小的欄位元素向量，定義在 [BLS12-381](/ethereum/cryptography/bls12-381/) 的標量域 $\mathbb{F}_p$ 上：
...
每個 blob 對應一個 [KZG Commitments](/ethereum/advanced/kzg-commitments/)：
...
Blob base fee 用類似 [EIP-1559 費用市場](/ethereum/accounts/eip-1559/) 的指數調整機制：
...
[區塊 Header](/ethereum/consensus/block-header/) 新增兩個欄位：

- `blob_gas_used`：區塊中所有 blob 消耗的 gas
- `excess_blob_gas`：累計超額 blob gas（用於計算 blob base fee）

[區塊結構](/ethereum/consensus/block-structure/) 的 Beacon Block sidecar 攜帶實際的 blob 資料和 KZG proof。
...
2. KZG commitment 的有效性（[KZG Commitments](/ethereum/advanced/kzg-commitments/) verification）
...
新增地址 `0x0A` 的 [Precompiled Contracts](/ethereum/advanced/precompiled-contracts/)，執行 KZG point evaluation：
...
- 每個 blob 被編碼為多項式，透過 [KZG Commitments](/ethereum/advanced/kzg-commitments/) 產生額外的 erasure-coded 列（columns）
...
- [KZG Commitments](/ethereum/advanced/kzg-commitments/) - Blob 的 polynomial commitment scheme
- [BLS12-381](/ethereum/cryptography/bls12-381/) - KZG commitment 使用的橢圓曲線
- [區塊結構](/ethereum/consensus/block-structure/) - Blob 相關欄位在 Execution Payload 中的位置
- [區塊 Header](/ethereum/consensus/block-header/) - blob_gas_used 和 excess_blob_gas 欄位
- [EIP-1559 費用市場](/ethereum/accounts/eip-1559/) - Blob gas 定價機制的類比
- [Precompiled Contracts](/ethereum/advanced/precompiled-contracts/) - Point evaluation precompile（0x0A）
- [Beacon Chain](/ethereum/consensus/beacon-chain/) - Blob sidecar 的傳播與暫存
- [Verkle Trees](/ethereum/advanced/verkle-trees/) - 另一個朝向 statelessness 的技術方向
- [交易構建](/ethereum/transaction-lifecycle/transaction-construction/) - Blob transaction 的建構流程
- [交易生命週期](/ethereum/transaction-lifecycle/transaction-lifecycle/) - Blob transaction 的完整生命週期
